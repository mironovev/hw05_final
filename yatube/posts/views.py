...
# Пока оставлю, исправлю завтра.
# Вообще отказался из-за того, что подписка/отписка с кэшированием криво
# работала (кнопка обновлялась только через 20 секунд). Если key_prefix
# именно для этого - верну декораторы, иначе просто удалю т.к. и так через
# кэширование в шаблонах работает :/
def index(request):
